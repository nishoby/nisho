    ...
    >
        ...
        <el-form-item prop="tags">
            ...
            <!-- Падказка знізу: тэгі, якімі карыстаюцца часцей за ўсё.
                 ...
                 набору рукамі. «18+», «мацюкі» і «сэкс» тут не выпадкова —
                 па іх сайт хавае дарослае ад тых, хто яго выключыў. -->
...
import { SUGGESTED_TAGS } from './suggested-tags.js';
...
function toggleTag(name, on) {
    if (on) {
        if (!hasTag(name)) {
            // Дакладна так, як напісана ў спісе. Раней тут першая літара рабілася
            // вялікай — і кожны націск ствараў у базе двайнік ужо існага тэга:
            // «Інтэрнэт» побач з «інтэрнэт», у якім ляжалі 23 словы.
            new_term.tags.push(name);
        }
